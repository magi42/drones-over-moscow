...
Run these commands from this `web/` directory:
...
Run `npm run build` to generate the deployable site in `dist/`. Run
`npm run package` to create `release/drones-over-moscow/` and
`drones-over-moscow.tar.gz`. All output paths are relative to `web/`.
